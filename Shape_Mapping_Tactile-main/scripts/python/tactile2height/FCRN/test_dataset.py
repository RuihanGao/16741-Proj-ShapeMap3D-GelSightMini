...
from loader import GelDataLoader
...
# the directory saving the data loading files
save_path = osp.join('..','..','..','..','..','..','dataset')

test_data_file = osp.join(save_path,'test_data.txt')
test_label_file = osp.join(save_path,'test_label.txt')

print("Loading data......")
test_loader = GelDataLoader(test_data_file, test_label_file)
...
with torch.no_grad():
    for input, gt in test_loader:
        ...
        # save generated height maps and ground truth maps
        np.save('../../../../../../results/test/gt/Test_gt_depth_{:05d}.npy'.format(idx), input_gt_depth_image)
        np.save('../../../../../../results/test/pred/Test_pred_depth_{:05d}.npy'.format(idx), pred_depth_image)
...
#         plot.imsave('../outputs/test/gt/Test_gt_depth_{:05d}.png'.format(idx), input_gt_depth_image, cmap="viridis")
#         plot.imsave('../outputs/test/pred/Test_pred_depth_{:05d}.png'.format(idx), pred_depth_image, cmap="viridis")
